...
- `DELETE /tasks/:id`
- `GET /goals` — все цели текущего пользователя, включая выполненные и корзину.
- `POST /goals` — `{ id, title }`; повтор с теми же ID и названием не создаёт дубликат.
- `PATCH /goals/:id` — название или выполнение, с `baseUpdatedAt`. При редактировании названия клиент также передаёт `baseTitle`: параллельное изменение оценок не блокирует переименование, а изменившееся название даёт `409`.
- `PATCH /goals/:id/priority-score` — `{ field: "priority", delta: -1 | 1 }`, атомарное изменение единого приоритета от 0 без продуктового верхнего лимита. Старые поля `importance` / `urgency` / `overdue` принимаются для совместимости со старыми клиентами.
- `PATCH /goals/:id/priority` — `{ targetIndex }`, порядок во Входящих.
- `DELETE /goals/:id` — перемещение в корзину; `POST /goals/:id/restore` — восстановление во Входящие.
- `GET /earnings`
- `POST /earnings`
- `PATCH /earnings/:id`
- `DELETE /earnings/:id`
...
- Раздел «Приоритеты» использует тот же набор задач: группы 1–9 имеют лимит по номеру,
  нераспределённые задачи находятся во «Входящих», а выполненные доступны в отдельном списке.
- Вес задачи считается как сумма важности и срочности (по 0–9 баллов), а группы 1–9
  автоматически пересобираются через `PATCH /tasks/:id/priority-score`.
- Порядок и перенос сохраняются через `PATCH /tasks/:id/priority`: drag-and-drop подстраивает
  вес и стабильный порядок, заполненные группы сдвигают более слабые задачи ниже, а после 45-го
  места задача возвращается во «Входящие».
...
Вкладка «Цели» использует отдельную таблицу `goals` (миграция
`20260913120000_add_goals`). Существующие задачи не копируются и не изменяются.
Цели не содержат таймеров, жизней и подзадач, не входят в календарь и статистику задач.
В интерфейсе целей нет чекбоксов; вместо трёх оценок и веса показан один «Приоритет».
Существующие оценки отображаются как сумма. При первом изменении единого приоритета
сервер записывает новую сумму в `priority_importance`, обнуляя два других поля только
этой цели. Миграция и массовая перезапись данных не нужны. При равных суммах сохраняется
порядок `priority_rank`, затем время создания и ID; категории старых оценок не влияют
на место цели. Новый клиент требует обновлённого backend для `field: "priority"`;
backend нужно выпустить раньше frontend. У задач остаются три оценки по 0–9.
Группы и лимит 45 мест считаются отдельно; при восстановлении сохраняются оценки,
но цель возвращается во Входящие. Статистика целей считает завершения за 7/30/90
календарных дней в часовом поясе браузера, исключая корзину и повторно открытые цели.
Состояние целей загружается с сервера и обновляется раз в 30 секунд / при фокусе окна;
локальный кэш задач не используется. Все маршруты используют текущего авторизованного
пользователя, изменения и перерасчёт групп выполняются в одной транзакции.

Для выпуска целей нужно применить новую миграцию и обновить backend перед frontend.
На старом backend вкладка показывает сообщение о недоступности и кнопку повтора.
Откат приложения не требует удаления таблицы: она совместима со старой версией,
которая продолжает использовать только `tasks`. Не удаляйте таблицу после появления
пользовательских целей. Локальный `npm run check` проверяет CRUD, лимит 45 мест,
конкурентные оценки, конфликты и изоляцию пользователей через `scripts/goals-smoke.mjs`.
