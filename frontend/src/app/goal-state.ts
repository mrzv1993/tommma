...
export function projectGoals(snapshot: GoalItem[], actions: GoalAction[]) {
  const goals = snapshot.map(goal => ({ ...goal }))
  for (const action of actions) {
    const goal = goals.find(row => row.id === action.id && !row.completed && !row.deletedAt)
    if (!goal) continue
    if (action.type === 'score') {
      const field = ({ importance: 'priorityImportance', urgency: 'priorityUrgency', overdue: 'priorityOverdue' } as const)[action.field]
      goal[field] = Math.max(0, Math.min(9, goal[field] + action.delta))
      const ranked = goal.priorityImportance + goal.priorityUrgency + goal.priorityOverdue > 0
...
export function useGoalState() {
  ...
  const bindings = computed(() => ({
    ...
    addTask,
    adjustScore: (id: string, field: 'importance' | 'urgency' | 'overdue', delta: -1 | 1) => mutate({ type: 'score', id, field, delta }),
