...
test('PostgreSQL: leases, retries, concurrency, budget, hierarchy and isolation', { skip: !safeDatabase }, async () => {
  ...
  try {
    ...
    assert.ok((await focus.start(user.id,missing.id,randomUUID())).sessionId)
    await focus.pause(user.id,missing.id)
    await assert.rejects(focus.split(user.id,missing.id,{children:[{id:randomUUID(),title:'A'},{id:randomUUID(),title:'B'}]}), /после 90/)
    ...
    const children = [1,2].map(i => ({id:randomUUID(), title:`Child ${i}`}))
    await assert.rejects(focus.split(user.id,legacy.id,{children:children.slice(0,1)}), /минимум две/)
    await focus.split(user.id,legacy.id,{children})
    await focus.split(user.id,legacy.id,{children})
    ...
    assert.equal(child.doneWhen,'')
    assert.ok((await focus.start(user.id,child.id,randomUUID())).sessionId)
    await focus.pause(user.id,child.id)
    await prisma.task.update({where:{id:child.id},data:{focusSpentMs:BUDGET_MS}})
    await focus.split(user.id,child.id,{children:[{id:randomUUID(),title:'Nested A'},{id:randomUUID(),title:'Nested B'}]})
